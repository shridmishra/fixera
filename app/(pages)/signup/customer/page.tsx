...
import { ArrowLeft, User, Mail, Phone, Lock, MapPin, Building2, FileText, Loader2, CheckCircle2, XCircle, AlertCircle } from "lucide-react"
...
export default function CustomerSignupPage() {
  ...
  const { isLoaded: mapsLoaded, validateAddress } = useGoogleMaps()
  ...
  const handleAddressChange = (fullAddress: string, placeData?: PlaceData) => {
    ...
    if (placeData?.coordinates && placeData?.address_components) {
      console.log('✅ Using place data from autocomplete - no API call needed')

      // Extract city, country, postal code from address components
      const city = placeData.address_components.find((c: any) =>
      ...
      const postalCode = placeData.address_components.find((c: any) =>
        c.types.includes('postal_code')
      )?.long_name || ''

      // Auto-fill the fields with data from Google Places
      setFormData(prev => ({
        ...prev,
        city: city || prev.city,
